...
;;Produce true if Pattern match with Los like if latter,numberic match with los "A" "N" respectivly. false other wise
(check-expect (pattern-match? empty empty) true)
(check-expect (pattern-match? (list "A" "N") empty) false)
(check-expect (pattern-match? empty (list "V" "6" "T" "1")) true)
(check-expect (pattern-match? (list "A" "N" "N" "N") (list "S" "0" "2" "6" "1" "T" "U" "R" "T" )) true)
(check-expect (pattern-match? (list "A" "N" "N" "N") (list "V" "6")) false)
(check-expect (pattern-match? (list "A" "A") (list "V" "6" "T" "1")) false)
(check-expect (pattern-match? (list "A" "N" "N" "N" "N" "N") (list "S" "0" "2" "6" "1" "2" "3" "5" "6")) true)
(check-expect (pattern-match? (list "A" "N" "N" "N" "A" "N") (list "S" "0" "2" "6" "T")) false)



;(define (pattern-match? PATTERN1 LOS1) false)alphabetic?


(define (pattern-match? PATTERN1 LOS1)
  (cond [(empty? PATTERN1) true]
        [(empty? LOS1) false]
        [else
         (and (if (string=? "A" (first PATTERN1))
                  (alphabetic? (first LOS1))
                  (numeric? (first LOS1)))
              (pattern-match? (rest PATTERN1) (rest LOS1)))]))